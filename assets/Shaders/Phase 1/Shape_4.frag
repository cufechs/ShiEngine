...
void main() {
    ...
    vec2 uv;
    uv.x = (gl_FragCoord.x - MousePos.x)/1280;
    uv.y = (gl_FragCoord.y - MousePos.y)/(1280 * 0.85);

    float distance = length(uv);

    if(distance < 0.12 && distance > 0.08 && !(uv.x > abs(uv.y)) ||
    (uv.x > 0.045 && uv.x < 0.085 && uv.y < 0.0 && uv.y > -0.085)){
