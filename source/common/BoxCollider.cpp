...
bool ShiEngine::BoxCollider::CollidesWith(ShiEngine::BoxCollider* BC) //Pass start and end vectors
{
    glm::vec3 SV = GetStartVector();
    glm::vec3 EV = GetEndVector();
    glm::vec3 SV2 = BC->GetStartVector();
    glm::vec3 EV2 = BC->GetEndVector();
    glm::vec3 Center1 = (SV2 + EV2)/2.0f;
    glm::vec3 Center2 = (SV + EV)/2.0f;

    if(abs(Center1.x - Center2.x) > (abs(Center1.x - SV2.x) + abs(Center2.x - SV.x)))
        return false;
    if(abs(Center1.y - Center2.y) > (abs(Center1.y - SV2.y) + abs(Center2.y - SV.y)))
        return false;
    if(abs(Center1.z - Center2.z) > (abs(Center1.z - SV2.z) + abs(Center2.z - SV.z)))
        return false;

    if(!IsTrigger && !BC->IsTrigger) {
